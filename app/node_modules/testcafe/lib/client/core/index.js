...
(function () {
    ...
    ***************************************************************************** */
    ...
    var isVisibilityHiddenNode = function (node) {
        return !!adapter.findParent(node, true, function (ancestor) {
            return adapter.isElementNode(ancestor) && adapter.getStyle(ancestor, 'visibility') === 'hidden';
        });
    };
    var isHiddenNode = function (node) {
        return !!adapter.findParent(node, true, function (ancestor) {
            return adapter.isElementNode(ancestor) && adapter.getStyle(ancestor, 'display') === 'none';
        });
    };
    ...
    function isElementVisible$1(el) {
        ...
        if (!adapter.isContentEditableElement(el)) {
            var elementRectangle = adapter.getElementRectangle(el);
            if (elementRectangle.width === 0 || elementRectangle.height === 0)
                return false;
        }
        if (adapter.isMapElement(el)) {
            var mapContainer = adapter.getMapContainer(adapter.closest(el, 'map'));
            return mapContainer ? isElementVisible$1(mapContainer) : false;
        }
        if (adapter.isSelectVisibleChild(el)) {
            var select = adapter.getSelectParent(el);
            var childRealIndex = adapter.getChildVisibleIndex(select, el);
            var realSelectSizeValue = adapter.getSelectElementSize(select);
            var topVisibleIndex = Math.max(adapter.getScrollTop(select) / adapter.getOptionHeight(select), 0);
        ...
        return hasDimensions(el) && adapter.getStyle(el, 'visibility') !== 'hidden';
    }
